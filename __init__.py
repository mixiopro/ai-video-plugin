...
def _blocked_import(name, *args, **kwargs):
    root = name.split(".")[0]
    # Serve stubs for diffusers/torch, block other local ML libs
    if root == "diffusers":
        mod = sys.modules.get("diffusers")
        if mod is None:
            # Defer to later stub registration, but ensure import doesn't crash
            return ModuleType("diffusers")
        return mod
    if root == "torch":
        ...
        return mod
    if root in {"transformers", "deepspeed", "accelerate", "xformers"}:
        raise ImportError("Local ML libraries are disabled; use providers.")
...
class FalPipelineShim:
    def __init__(self, *, model_id: str | None = None):
        from .pipelines.flux_fal_adapter import FluxFalAdapter
        self._adapter = FluxFalAdapter(model_id=model_id)
        ...
        class _VAE:
            ...
            @property
            def config(self):
                return SimpleNamespace(shift_factor=0, scaling_factor=1)

        self.vae = _VAE()

    # no-op device/optimization methods for compatibility
    def to(self, *_, **__):
        return self

    def enable_model_cpu_offload(self, *_, **__):
        return None

    def enable_vae_slicing(self, *_, **__):
        return None
    ...
    def __call__(self, *args, **kwargs):
        ...
        result = self._adapter(prompt or "", arguments=arguments)
        images = result.get("images") if isinstance(result, dict) else None
        if images and isinstance(images, list) and images and isinstance(images[0], dict) and images[0].get("url"):
            try:
                pil = _download_image(images[0]["url"])
...
def _extract_first_frame(video_path: str, *, width: int | None = None, height: int | None = None):
    ...
    try:
        ...
        scale = []
        if width and height:
            scale = ["-vf", f"scale={width}:{height}:force_original_aspect_ratio=decrease"]
        cmd = [
            "ffmpeg",
            "-y",
            "-i",
...
class VideoFalShim:
    def __init__(self, *, model_id: str | None = None):
        from .pipelines.video_fal_adapter import VideoFalAdapter
        self._adapter = VideoFalAdapter(model_id=model_id)
    ...
    def __call__(self, *args, **kwargs):
        ...
        # Try to surface frames[0] compatibility
        url = None
        if isinstance(result, dict):
            # Common Fal patterns: {"video":{"url":...}} or top-level list/urls
        ...
        if url:
            try:
                vpath = _download_file(url, ".mp4")
                pil = _extract_first_frame(vpath, width=kwargs.get("width"), height=kwargs.get("height"))
            except Exception:
                pil = None
        if pil is None:
            try:
                ...
                pil = Image.new("RGB", (512, 512), (0, 0, 0))

        return SimpleNamespace(frames=[pil])
...
class AudioFalShim:
    def __init__(self, *, model_id: str | None = None):
        from .providers.media_fal import generate_audio
        self._run = generate_audio
    ...
    def __call__(self, *args, **kwargs):
        ...
        if "prompt" in arguments:
            del arguments["prompt"]
        result = self._run(model_id=None, arguments={"prompt": prompt, **arguments})
        ...
        if not url:
            return SimpleNamespace(audios=[_FakeTensor(np.zeros((2, 1), dtype=np.float32))])

        try:
            # Download and convert to wav via ffmpeg, then read with wave
            src = _download_file(url, ".audio")
            wav_path = src + ".wav"
            cmd = ["ffmpeg", "-y", "-i", src, wav_path]
            subprocess.run(cmd, check=True, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
            import wave
            wf = wave.open(wav_path, "rb")
            self.vae.sampling_rate = wf.getframerate()
            ...
            arr = arr.reshape(-1, n_channels).T  # (channels, samples)
            return SimpleNamespace(audios=[_FakeTensor(arr)])
        except Exception:
            return SimpleNamespace(audios=[_FakeTensor(np.zeros((2, 1), dtype=np.float32))])


# Provide minimal diffusers stubs so legacy branches using Flux work without local models
...
class _FluxPipeline:
    ...
    def from_pretrained(cls, *_, **kwargs):
        model_id = kwargs.get("transformer") and getattr(kwargs.get("transformer"), "model_id", None)
        return FalPipelineShim(model_id=model_id)
...
setattr(diffusers_stub, "BitsAndBytesConfig", _BitsAndBytesConfig)
setattr(diffusers_stub, "FluxTransformer2DModel", _FluxTransformer2DModel)
setattr(diffusers_stub, "FluxPipeline", _FluxPipeline)
setattr(diffusers_stub, "FluxKontextPipeline", _FluxKontextPipeline)

# Additional image pipelines mapped to Fal image shim
class _QwenImagePipeline(_FluxPipeline):
    pass

class _QwenImageImg2ImgPipeline(_FluxPipeline):
    pass

setattr(diffusers_stub, "QwenImagePipeline", _QwenImagePipeline)
setattr(diffusers_stub, "QwenImageImg2ImgPipeline", _QwenImageImg2ImgPipeline)

_VIDEO_PIPELINE_HINTS = {
    "CogVideoXPipeline",
...
def _make_pipeline_factory(name: str):
    class _GenericPipeline:
        @classmethod
        def from_pretrained(cls, *_, **kwargs):
            if ("Video" in name) or (name in _VIDEO_PIPELINE_HINTS):
                return VideoFalShim()
            if ("Audio" in name) or (name in _AUDIO_PIPELINE_HINTS):
                return AudioFalShim()
            return FalPipelineShim()
    _GenericPipeline.__name__ = name
    return _GenericPipeline

def __getattr__(name: str):
    if name.endswith("Pipeline") and not hasattr(diffusers_stub, name):
        factory = _make_pipeline_factory(name)
        setattr(diffusers_stub, name, factory)
        return factory
    raise AttributeError(name)

setattr(diffusers_stub, "__getattr__", __getattr__)

sys.modules.setdefault("diffusers", diffusers_stub)

# minimal torch stub used by legacy branches (e.g., torch.bfloat16 checks)
...
setattr(torch_stub, "backends", _Backends())
sys.modules.setdefault("torch", torch_stub)

# minimal transformers stub for BitsAndBytesConfig & text encoders referenced
transformers_stub = ModuleType("transformers")

class _TransformersBitsAndBytesConfig:
    def __init__(self, *_, **__):
        pass
...
setattr(transformers_stub, "BitsAndBytesConfig", _TransformersBitsAndBytesConfig)
setattr(transformers_stub, "Qwen2_5_VLForConditionalGeneration", _QwenTextEncoder)
...
try:
    ...
    if (not REMOTE_ONLY) and os_platform == "Darwin":
...
def install_modules(self):
    ...
    def install_module(name, package=None, use_git=False):
        ...
        # Skip heavy local ML deps in remote-only mode
        if REMOTE_ONLY:
            blocked = {
                "torch","torchvision","torchaudio","xformers","diffusers","transformers",
                "sentencepiece","safetensors","bitsandbytes","deepspeed","accelerate",
                "sageattention","timm","tokenizers","protobuf","imageio","imWatermark",
                "controlnet-aux","laion-clap","whisperspeech","parler-tts","mmaudio",
                "stable-audio-tools","resemble-perth","s3tokenizer","conformer","jax","jaxlib",
            }
            pkg_str = str(package)
            nm_str = str(name)
            if nm_str in blocked or any(b in pkg_str for b in blocked):
                print(f"Remote-only: skipping install of {package}")
                return
        try:
            subprocess.check_call([
                pybin, "-m", "pip", "install", package,
    ...
    # Common modules (trimmed for REMOTE_ONLY)
    if REMOTE_ONLY:
        common_modules = [
            ("requests", "requests"),
            ("PIL", "pillow"),
    ...
    if (not REMOTE_ONLY) and os_platform == "Windows":
        windows_modules = [
    ...
    if (not REMOTE_ONLY) and version.parse(".".join(map(str, python_version[:3]))) >= version.parse("3.8"):
